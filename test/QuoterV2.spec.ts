...
const isEVM = network.name === 'hardhat'
  ...
  describe('quotes', () => {
    ...
    describe('#quoteExactInput', () => {
      it('0 -> 2 cross 2 tick', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(205756) // OVM gas estimation is not deterministic, so only run on EVM
      ...
      it('0 -> 2 cross 2 tick where after is initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(165741) // OVM gas estimation is not deterministic, so only run on EVM
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('0 -> 2 cross 1 tick', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(165703) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(1)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('0 -> 2 cross 0 tick, starting tick not initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(118532) // OVM gas estimation is not deterministic, so only run on EVM
      ...
      it('0 -> 2 cross 0 tick, starting tick initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(142400) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(1)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 2', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(196103) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(2)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 2 where tick after is initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(196111) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(2)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 0 tick, starting tick initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(112809) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(0)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 0 tick, starting tick not initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(108884) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(0)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 1', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(112472) // OVM gas estimation is not deterministic, so only run on EVM
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('0 -> 2 -> 1', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(305724) // OVM gas estimation is not deterministic, so only run on EVM
    ...
    describe('#quoteExactInputSingle', () => {
      it('0 -> 2', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(205738) // OVM gas estimation is not deterministic, so only run on EVM
      ...
      it('2 -> 0', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(196075) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossed).to.eq(2)
        expect(quote).to.eq(9871)
    ...
    describe('#quoteExactOutput', () => {
      it('0 -> 2 cross 2 tick', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(204762) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList.length).to.eq(1)
        expect(initializedTicksCrossedList[0]).to.eq(2)
      ...
      it('0 -> 2 cross 2 where tick after is initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(164991) // OVM gas estimation is not deterministic, so only run on EVM
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('0 -> 2 cross 1 tick', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(164953) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList.length).to.eq(1)
        expect(initializedTicksCrossedList[0]).to.eq(1)
      ...
      it('0 -> 2 cross 0 tick starting tick initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(141919) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList.length).to.eq(1)
        expect(initializedTicksCrossedList[0]).to.eq(1)
      ...
      it('0 -> 2 cross 0 tick starting tick not initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(118071) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList.length).to.eq(1)
        expect(initializedTicksCrossedList[0]).to.eq(0)
      ...
      it('2 -> 0 cross 2 ticks', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(196209) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList.length).to.eq(1)
        expect(initializedTicksCrossedList[0]).to.eq(2)
        expect(amountIn).to.eq(15273)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 2 where tick after is initialized', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(196203) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(2)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 0 cross 1 tick', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(156408) // OVM gas estimation is not deterministic, so only run on EVM
        expect(initializedTicksCrossedList[0]).to.eq(1)
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('2 -> 1', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(113049) // OVM gas estimation is not deterministic, so only run on EVM
        expect(sqrtPriceX96AfterList.length).to.eq(1)
      ...
      it('0 -> 2 -> 1', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(305324) // OVM gas estimation is not deterministic, so only run on EVM
        expect(sqrtPriceX96AfterList.length).to.eq(2)
    ...
    describe('#quoteExactOutputSingle', () => {
      it('0 -> 1', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(122770) // OVM gas estimation is not deterministic, so only run on EVM
      ...
      it('1 -> 0', async () => {
        ...
        if (isEVM) expect(gasEstimate).to.eq(113654) // OVM gas estimation is not deterministic, so only run on EVM
        expect(amountIn).to.eq(9981)
        expect(initializedTicksCrossed).to.eq(0)
